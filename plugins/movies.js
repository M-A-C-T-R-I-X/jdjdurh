...
async function sea(conn,chat,mek,q,reply,type,remotejids){
        ...
        if (q.startsWith(cine.site)){
            ...
            if(cineresponse?.result?.data){
             const dataAfterSachibot = cineresponse.result.data; 
              ...
              let cot = `*▣|𝙼𝙾𝚅𝙸𝙴 𝙳𝙾𝚆𝙽𝙻𝙾𝙰𝙳𝙴𝚁 𝚂𝚈𝚂𝚃𝙴𝙼|▣*`+
              `\n`+
              `\n`+
              `*╭───「 ᴍᴏᴠɪᴇ ɪɴꜰᴏ 」───*`+`\n`+
              ` *| ▣➮ ${title}*`+`\n`+
    ...
    async function sendinfo(conn,chat,mek,type,reply,q){
      ...
      const output = `*☘ Tιтle :- _${showInfo.maintitle}_*`+`\n`+
      `● *━━━「ɪɴꜰᴏʀᴍᴀᴛɪᴏɴꜱ」━━━* ●`+`\n`+
      ...
      const output = `*☘ Tιтle :- _${movie.mainDetails.maintitle}_*`+`\n`+
      `● *━━━「ɪɴꜰᴏʀᴍᴀᴛɪᴏɴꜱ」━━━* ●`+`\n`+
      ...
      const output = `*☘ Tιтle :- _${showInfo.title}_*`+`\n`+
      `● *━━━「ɪɴꜰᴏʀᴍᴀᴛɪᴏɴꜱ」━━━* ●`+`\n`+
      ...
      const output = `*☘ Tιтle :- _${movieInfo.title}_*`+`\n`+
      `● *━━━「ɪɴꜰᴏʀᴍᴀᴛɪᴏɴꜱ」━━━* ●`+`\n`+
    ...
    const moviesend = async (reply,title, caption, url, conn, mek, chats) => {
      ...
      if(chats?.length === 1 && chats[0] === where){
        const mass = await conn.sendMessage(where, { document: mydata, mimetype: mime, fileName: '「DIMENSION-X」~'+title + '.'+ext, caption: caption }, { quoted: mek });
        await conn.sendMessage(where, { react: { text: "✓", key: mass.key } });
        ...
        await Promise.all(chats.map(async (chat) => {
          const mass = await conn.sendMessage(chat, { document: mydata, mimetype: mime, fileName: '「DIMENSION-X」~'+title + '.'+ext, caption: caption });
           await conn.sendMessage(chat, { react: { text: "✓", key: mass.key } });
      ...
      if(chats?.length === 1 && chats[0] === where){
        const mass = await conn.sendMessage(where, { document: mdata, mimetype: mime, fileName: '「DIMENSION-X」~'+title + '.'+ext, caption: caption }, { quoted: mek });
        await conn.sendMessage(where, { react: { text: "✓", key: mass.key } });
        ...
        if(config.DOWNLOADSAPI !== ''){
         ...
         const sen = await fetchJson(`${config.DOWNLOADSAPI}${bot}/download?index=${indexNumber}&jid=${where}&done=true&target=true&${sinsub.apikey}${config.DEVAPIKEY}`);   
          ...
          await Promise.all(chats.map(async (chat) => {
            const mass = await conn.sendMessage(chat, { document: mdata, mimetype: mime, fileName: '「DIMENSION-X」~'+title + '.'+ ext, caption: caption });
              await conn.sendMessage(chat, { react: { text: "✓", key: mass.key } });
